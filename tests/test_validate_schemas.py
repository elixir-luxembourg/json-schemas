    ...
    def test_validation(self):
        for schema_filename, dataset_filename in schema_testdata_filename_map:
            ...
            data_filepath = os.path.abspath(
                os.path.join(testdata_dir, dataset_filename)
            )
            ...
            for item in data:
                try:
                    ...
                    self.fail(
                        f"JSONSchema {schema_filepath} is not valid - SchemaError."
                    )
                except ValidationError:
                    self.fail(
                        f"JSONSchema {schema_filepath} is not valid - failed on test data {data_filepath}"
                    )
